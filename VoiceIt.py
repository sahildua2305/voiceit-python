import hashlib,urllib2,json,requests
...
class VoiceIt:
    ...
    def createUser(self,mail,passwd,firstName,lastName,phone1="",phone2="",phone3=""):
        ...
        headers = {'Content-Type':'application/json',"VsitEmail":email,"VsitPassword":password,"VsitDeveloperId":self.developerId,"VsitFirstName":firstName,"VsitLastName":lastName,"VsitPhone1":phone1,"VsitPhone2":phone2,"VsitPhone3":phone3}
        response = ""
        try:
    ...
    def createEnrollment(self,mail,passwd,pathToEnrollmentWav):
        email = mail
        password = self.getSHA256(passwd)
        ...
        response = ""
        try:
            response = requests.post(self.urlEnrollments,headers=headers,data=wavData)
            return response.text
        except requests.exceptions.HTTPError, e:
            return e.read()
    
    def createEnrollmentByWavURL(self,mail,passwd,urlToEnrollmentWav):
        email = mail
        password = self.getSHA256(passwd)
        headers = {'Content-Type':'audio/wav',"VsitEmail":email,"VsitPassword":password,"VsitDeveloperId":self.developerId,"VsitwavURL":urlToEnrollmentWav}
        response = ""
        try:
            response = requests.post(self.urlEnrollments+"/bywavurl",headers=headers)
            return response.text
        except requests.exceptions.HTTPError, e:
            return e.read()
    ...
    def deleteEnrollment(self,mail,passwd,enrollmentId):
        ...
        response = ""
        try:
            response = requests.delete(self.urlEnrollments+ "/"+ enrollmentId,headers=headers)
            return response.text
        except requests.exceptions.HTTPError, e:
            return e.read()
            
    def authentication(self,mail,passwd,pathToAuthenticationWav,accuracy,accuracyPasses, accuracyPassIncrement, confidence):
        email = mail
        password = self.getSHA256(passwd)
        wavData = open(pathToAuthenticationWav, 'rb').read()
        headers = {'Content-Type':'audio/wav',"VsitEmail":email,"VsitPassword":password,"VsitDeveloperId":self.developerId,
        "VsitAccuracy":accuracy, "VsitAccuracyPasses":accuracyPasses,"VsitAccuracyPassIncrement":accuracyPassIncrement,"VsitConfidence":confidence}
        response = ""
        try:
            response = requests.post(self.urlAuthentication,headers=headers,data=wavData)
            return response.text
        except requests.exceptions.HTTPError, e:
            return e.read()
            
    def authenticationByWavURL(self,mail,passwd,urlToAuthenticationWav,accuracy,accuracyPasses, accuracyPassIncrement, confidence):
        email = mail
        password = self.getSHA256(passwd)
        ...
        response = ""
        try:
            response = requests.post(self.urlAuthentication+"/bywavurl",headers=headers)
            return response.text
        except requests.exceptions.HTTPError, e:
            return e.read()
